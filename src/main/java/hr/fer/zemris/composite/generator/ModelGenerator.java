...
public class ModelGenerator {
  ...
  private List<AbstractNode> connectNodes(final List<AbstractNode> nodes, final int currentLevel, final int depthK,
    ...
    final int p = calculateNumberOfEdges(levelEdges.get(currentLevel));
  ...
  private int calculateNumberOfEdges(final Map<AbstractNode, Integer> map) {
    int counter = 0;
  ...
  private List<AbstractNode> createNodes(final int numberOfNodes) {
    ...
    for (int i = 0; i < numberOfNodes; i++) {
      final AbstractNode node = NodeType.get(nodeTypeDistribution.sample()).newInstance();
      node.setId(nextId());
