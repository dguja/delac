...
public class ReliabilityImpactCalculator {
  ...
  public static List<ReliabilityImpactVector> calculate(Dataset dataset) {
    ...
    for (Model model : dataset.getModels()) {
      ...
      for (int i = 0; i < model.getInputs().size(); ++i) {
        ...
        relImpact[idToIndex.get(inputNode.getId())] = calculateReliabilityImpact(inputNode, outputNode, oldReliability);
      }
  ...
  private static double calculateReliabilityImpact(InputNode inputNode, OutputNode outputNode, double oldReliability) {
